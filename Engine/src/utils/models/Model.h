...
namespace TESLA
{
    ...
    public:
            ...
            :m_viewMatrix(model.m_viewMatrix), m_projectionMatrix(model.m_projectionMatrix)
        {
            ...
            m_instancedPositions = model.m_instancedPositions;
        }
        
        Model(const char* fileName, const uint32_t& shaderProgram, uint32_t texture, glm::mat4& viewMatrix, glm::mat4& projectionMatrix, int instanceCount, std::vector<glm::vec4> instancePositions)
            : m_viewMatrix(viewMatrix), m_projectionMatrix(projectionMatrix)
        {
            m_shaderProgram = shaderProgram;
            m_texture = texture;
            m_instanceCount = instanceCount;
            m_instancedPositions = instancePositions;
